...
class EnterPriseController extends Controller
{
    ...
    public function saveEnterprise(Request $request){ 
        ...
        if($validator->passes()){
            ...
            if(!empty(@$id) and !is_null(@$id)){
                ...
                $res = DB::table('plans_enterprise')->where('id',$id)->update($formdata);
                if(!empty($res)){
                ...
                $res = DB::table('plans_enterprise')->insertGetId($formdata);
                if(!empty($res)){
    ...
    public function showEnterPrise(Request $request)
    {
        if ($request->ajax()) {
    ...
    public function statusEnterprise(Request $request)
    {
        $id = $request->id;
        ...
        $row = DB::table('plans_enterprise')->where('id', $id)->update($data);
        if (empty($row)) {
    ...
    {
        $id = $request->id;
        $data = DB::table('plans_enterprise')->where('id', $id)->first();
    ...
    public function deleteEnterprise(Request $request)
    {
        ...
        $row = DB::table('plans_enterprise')->where('id', $id)->delete();
        if (empty($row)) {
