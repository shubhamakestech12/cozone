...
class CountryController extends Controller
{
    ...
    public function saveCountry(Request $request){ 
        ...
        if($validator->passes()){
            $formdata['name']   = $request->country_name;
            if(!empty($request->file('flag'))){
                $filename = uniqid().'.'.$request->file('flag')->getClientOriginalExtension(); 
                $request->file('flag')->move(public_path('uploads'), $filename);
                $formdata['flag'] = url('uploads').'/'.$filename;
            }

            if(!empty(@$id) and !is_null(@$id)){
                
                $res = Country::where('id',$id)->update($formdata);
                if(!empty($res)){
                    return response()->json(['msg'=>'Country Updated Successfully !','status'=>1]);
                }else{
                    return response()->json(['msg'=>"Can't Updated Country !",'status'=>2]);
                }
            }else{
                $res = Country::insertGetId($formdata);
                if(!empty($res)){
   ...
   // show data
    public function showCountry(Request $request){
        if($request->ajax()) {
           $data = Country::where(['is_deleted'=>1])
            ->get(['countries.name as country_name','countries.is_active','countries.id as id','countries.is_deleted']);
                ...
                ->filter(function ($instance) use ($request) {
                    if (!empty($request->get('search'))) {
                        $instance->collection = $instance->collection->filter(function ($row) use ($request) {
                            if (Str::contains(Str::lower($row['country_name']),$request->get('search'))){
                                return true;
                            }
    ...
    //  update status
    public function statusCountry(Request $request){
        $id = $request->id;
        ...
        $row = Country::where('id',$id)->update($data);
        if(empty($row)){
 ...
 //Update Function
    ...
    public function deleteCountry(Request $request){
        ...
        $row = Country::where('id',$id)->delete();

        if(empty($row)){
            return response()->json(['msg'=>"Can't Deleted Country !",'status'=>2]);
        }else{

          return response()->json(['msg'=>"Country Deleted Successfully !",'status'=>1]);
        }
    }//End if Function
