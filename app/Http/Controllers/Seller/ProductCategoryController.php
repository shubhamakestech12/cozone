...
class ProductCategoryController extends Controller
{
    ...
    public function saveCategory(Request $request){
            ...
            if($validator->passes()){
                ...
                if(!empty($id) and !is_null($id)){
                    ...
                    $res = ProductCategory::where('id',$id)->update($formdata);
                    if(!empty($res)){
                    ...
                    $res = ProductCategory::insertGetId($formdata);
                    if(!empty($res)){
    ...
    public function showCategory(Request $request){
        if($request->ajax()) {
           $data = ProductCategory::where('is_deleted',1)->get();
            return Datatables::of($data)
                ...
                ->addColumn('status', function($row){
                    if($row->is_active == 1){
                        return '<div class="userDatatable-content d-inline-block" rel="tooltip" title="Change Status" onclick="statusCategory('.$row->id.','.$row->is_active.')"> <span class="btn bg-opacity-success  color-success rounded-pill userDatatable-content-status active" id="status'.$row->id.'">Active</span> </div>';
                    }else{
                        return '<div class="userDatatable-content d-inline-block" rel="tooltip" title="Change Status" onclick="statusCategory('.$row->id.','.$row->is_active.')"> <span class="btn bg-opacity-warning  color-warning rounded-pill userDatatable-content-status active" id="status'.$row->id.'">Deactive</span> </div>';
                    }
                })
                ->addColumn('action', function($row){

                    return '<ul class="orderDatatable_actions mb-0 d-flex flex-wrap" style="min-width:90px;justify-content:unset;"> <li onclick="editCategory('.$row->id.')"> <a href="#" class="edit"> <svg  width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></a> </li> <li onclick="deleteCategory('.$row->id.')"> <a href="#" class="remove"> <svg  width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></a> </li> </ul>';
    ...
    public function deleteCategory(Request $request){
        ...
        $row = ProductCategory::where('id',$id)->delete($data);
        if(empty($row)){
    ...
    public function statusCategory(Request $request){
        ...
        $row = ProductCategory::where('id',$id)->update($data);
        if(empty($row)){
    ...
    public function checkSlug($slug=''){
        if(!empty($slug)){
            $resp = ProductCategory::where('slug',$slug)->first(['slug']);
